...
def plotOutput(P,k,times,title):
    ...
    plt.pyplot.xlabel('Node', fontsize=34)
    plt.pyplot.ylabel('Weight', fontsize=34)
    plt.pyplot.title(f'{title}: Optimal Solution for t={times[0]}', fontsize=36)
    plt.pyplot.legend()
    plt.pyplot.savefig(title)
    plt.pyplot.show()
...
if False:
    ...
    plt.pyplot.savefig("Graph Visualization")
    plt.pyplot.show()
...
if False:
    ...
    for i in range(4):
        ...
        ax.set_zlabel('Mass', fontsize=16, rotation=90)
        ax.tick_params(axis='x', labelsize=12)
        ax.tick_params(axis='y', labelsize=12)
        ax.tick_params(axis='z', labelsize=12)
        ax.set_title('Plot of Optimal Distributions for RGGs', fontsize=16)
        cbar=plt.pyplot.colorbar(surf)
        cbar.ax.tick_params(labelsize=14)
        cbar.ax.yaxis.offsetText.set_fontsize(14)
        plt.pyplot.savefig(f"RGG_n={num}_No_Data_Dist_{i}.png")
        plt.pyplot.show()
    ...
    for i in range(4):
        ...
        ax.tick_params(axis='y', labelsize=12)
        ax.tick_params(axis='z', labelsize=12)
        ax.set_title('Plot of Optimal Distributions for RGGs with Data', fontsize=16)
        cbar=plt.pyplot.colorbar(surf)
        cbar.ax.tick_params(labelsize=14)
        cbar.ax.yaxis.offsetText.set_fontsize(14)
        plt.pyplot.savefig(f"RGG_n={num}_With_Data_Dist_{i}.png")
        plt.pyplot.show()
#################################################
...
if False:
    ...
    cbar.ax.yaxis.offsetText.set_fontsize(14)
    plt.pyplot.title('Bay Area Road Network Optimal Distribution',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.show()
    plt.pyplot.savefig(f"Optimal_Distribution_3_k{k}_Modified_Bay.png")
...
if False:
    ...
    plt.pyplot.title('Bay Area Road Network Hard Partition',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.savefig(f"Hard_Partition_k{k}_Modified_Bay.png")
    plt.pyplot.show()
#################################################
...
if False:
    ...
    cbar.ax.tick_params(labelsize=16)
    cbar.ax.yaxis.offsetText.set_fontsize(14)
    plt.pyplot.title('Ithaca Road Network Optimal Distribution',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.show()
    plt.pyplot.savefig(f"Optimal_Distribution_k{k}_Modified_Ithaca.png")
...
if False:
    ...
    plt.pyplot.title('Ithaca Road Network Hard Partition',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.show()
    plt.pyplot.savefig(f"Hard_Partition_k{k}_Modified_Ithaca.png")
#################################################
...
if False:
    ...
    cbar.ax.tick_params(labelsize=16)
    cbar.ax.yaxis.offsetText.set_fontsize(14)
    plt.pyplot.title('Texas Road Network Optimal Distribution',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.show()
    plt.pyplot.savefig("Optimal_Distribution_1_k2_Modified_Texas.png")
...
if False:
    ...
    fig,ax=plt.pyplot.subplots(figsize=(8, 6))
    plt.pyplot.scatter(xdat, ydat, c=[color_map[i] for i in clustering], s=2, linestyle='None', marker='o')
    plt.pyplot.title('Texas Road Network Hard Partition',fontsize=16)
    ax.grid(True)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.pyplot.show()
    plt.pyplot.savefig("Hard_Partition_k2_Modified_Texas.png")
#################################################
...
if False:
    ...
    plt.pyplot.savefig("SPMS_by_Iteration_3_Initializations")
    plt.pyplot.show()
#################################################
...
def flowBetweenSlow(Start,End,G,p,t): #flow of probability between subsets Start -> End with distribution p
    A = nx.adjacency_matrix(G).todense()
    degrees = np.ravel(A@np.ones(G.number_of_nodes()))
    D = np.diag([degrees[i] for i in range(len(degrees))])
    T = np.linalg.inv(D)@A
    statDist=[x/(2*G.number_of_edges()) for x in degrees]
    piMat = np.tile(statDist, (G.number_of_nodes(), 1))
...
def n_largest_indices_old(arr, n):
    # Get the indices of the n largest elements
    indices = np.argpartition(arr, -n)[-n:]
    # Sort the indices by the corresponding element values in descending order
    return indices
...
def n_largest_indices_seeded(arr,prevCore):
    temp=arr.copy()
    for n in prevCore: #to not add the previous nodes
        temp[n]=-1
    added_node=n_largest_indices(temp,1)
    return np.append(prevCore,added_node)
...
def plotOutput(P,k,times,title):
    ...
    plt.title(f'{title}: Optimal Solution for t={times[0]}', fontsize=20)
...
def plot_by_C(arr,n,title):
    ...
    plt.xlabel('Core Size', fontsize=18)
    plt.ylabel('Metric Value', fontsize=18)
    plt.title(title, fontsize=20)
    plt.savefig(title)
    plt.show()

def corePeripheryMetricSlow(Core,Periphery,G,p,t):
    num=flowBetweenSlow(Core,Core,G,p,t)
    den=len(Core)+flowBetweenSlow(Periphery,Core,G,p,t)
    return num/den
...
def visualize_core(Core,x,y,groups,goi):
    ...
    ax2.set_title(f'Core Size: {len(Core)}', fontsize=24)
    ax2.set_xlabel('X', fontsize=18)
    ax2.set_ylabel('Y', fontsize=18)
    plt.tight_layout()
    plt.savefig(f'Data Points with Core (Core Size: {len(Core)})')
    plt.show()

def visualize_core_alt(Core1,Core2,Core3,x,y,groups,goi):
    fig, ax2 = plt.subplots(1, 1, figsize=(6, 6))
    groups2=groups.copy()
    for n in Core3:
        groups2[n]=5
    ...
    color_map2[goi]="yellow"
    colors2 = [color_map2[group] for group in groups2]
    ax2.scatter(x, y,c=colors2)
    plt.xlim(np.min(x[groups2==goi]-0.1), np.max(x[groups2==goi])+0.1)
    plt.ylim(np.min(y[groups2==goi]-0.1), np.max(y[groups2==goi])+0.1)
    ax2.set_title(f'Data Points with Core (Core Size: {len(Core1)})', fontsize=16)
    ax2.set_xlabel('X', fontsize=14)
    ax2.set_ylabel('Y', fontsize=14)
    plt.tight_layout()
    plt.savefig(f'Data Points with Core')
    plt.show()
...
def visualize_data(x,y,groups,goi):
    ...
    ax1.set_title('Data Points', fontsize=24)
    ax1.set_xlabel('X', fontsize=18)
    ax1.set_ylabel('Y', fontsize=18)
    plt.tight_layout()
    plt.savefig(f'Core Periphery RGG Data Points')
    plt.show()
...
if False:
    ...
    plot_by_C(testMetric,200,"Core-Periphery Metric by Core Size")
    print(bestCore)
    print(len(bestCore))
#################################################
...
if False:
    ...
    for i in range(n_trials):
        G,xdat,ydat,counts=eps_connectivity_graph(eps=0.22,var=0.4,n=2000,visual=False)
        k=4
        times=[5]
        Tt=computeTt(G,times)
        dat,P=maxMS(k,times,G,gamma=0)
...
if False:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(x_values, metric)
    plt.xlabel('Core Size', fontsize=14)
    plt.ylabel('Metric Value', fontsize=14)
    plt.title("Core-Periphery Metric by Core Size", fontsize=16)
    plt.savefig('Core-Periphery Metric on RGGs')
    plt.show()
...
if True:
    ...
    for j in range(len(indexset)):
        ...
        m=75000 #max size of acceptable core (583278) (583278/12 is approximately 50000)
        l=1000
        inc=25
        ...
        plt.pyplot.figure(figsize=(10, 6))
        plt.pyplot.plot(x_values[np.array(testMetric>0)], testMetric[np.array(testMetric>0)], '.')
        plt.pyplot.xlabel('Core Size', fontsize=16)
        plt.pyplot.ylabel('Value', fontsize=16)
        plt.pyplot.title("Core-Periphery Metric by Core Size", fontsize=18)
        plt.pyplot.show()
        plt.pyplot.savefig(f"Distribution_{indexset[j]}_k12_nLarg_Core_Periphery_Metric_Modified_Bay.png")
        ...
        plt.pyplot.title('Plot of Road Network with Optimal Core', fontsize=16)
        ax.grid(True)
        ax.set_xlabel('')
        ax.set_ylabel('')
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        plt.pyplot.show()
        plt.pyplot.savefig(f"Distribution_{[j]}_k12_nLarg_Core_Periphery_Modified_Bay.png")
    print(bestcoresizes)
#################################################
